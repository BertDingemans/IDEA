...
Namespace DLAFormfactory
    ...
    Public Class DLA2EAHelper
        ...
        ''' <summary>
        ''' Bouw recursief een lijst van package ids op op basis van de packageboomstructuur onder een rootpackage
        ''' </summary>
        ''' <param name="oPackage">Rootpackage voor opbouw van een lijst van package ids</param>
        ''' <returns>Lijst van packageids in een komma gescheiden lijst</returns>
        ...
        Public Shared Function String2File(ByVal text As String, ByVal strPath As String, Optional Mode As FileMode = FileMode.Append) As Boolean
            ...
            Try
                If System.IO.File.Exists(strPath) Then
                    fileStream = New FileStream(strPath, Mode, FileAccess.Write)
                Else
                    fileStream = New FileStream(strPath, FileMode.Create, FileAccess.Write)
                End If
                streamWriter = New StreamWriter(fileStream)
                ...
                Return True
            Catch ex As Exception
                MsgBox("Error in errorhandling, error log is not accessible, please contact the system administrator!")
