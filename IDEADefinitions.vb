...
Public Class IDEADefinitions
    ...
    Sub New()
        Me.LoadDefinitions()
        Me.LoadFromSettings(GetObject(, "EA.App").Repository)
    End Sub

    Sub New(Repository As EA.Repository)
        Me.LoadDefinitions()
        Me.LoadFromSettings(Repository)
    End Sub
    ...
    Public Function SaveSettings(Settings As String, statements As String, Repository As EA.Repository) As Boolean
        Dim oDT As DataTable
        Dim oElement As EA.Element
        oDT = DLA2EAHelper.SQL2DataTable(String.Format("SELECT object_id FROM t_object WHERE object_type = 'Artifact' AND name = 'IDEAConfig' AND version >= '{0}'", My.Application.Info.Version.ToString().Substring(0, 3)), Repository)
        If oDT.Rows.Count > 0 Then
            Dim oDR As DataRow
            oDR = oDT.Rows(0)
            oElement = Repository.GetElementByID(oDR("object_id"))
            Dim objDS As New DataSet2Repository()
            objDS.AddOrUpdateTaggedValue(oElement, "Settings", Settings, True)
            objDS.AddOrUpdateTaggedValue(oElement, "SQL-Statement", statements, True)
            oElement.Update()
        Else
            My.Settings.SettingsTable = Settings
            My.Settings.StatementsTable = statements
            My.Settings.Save()
        End If
        Return True
    End Function
    ...
    Sub LoadFromSettings(Repository As EA.Repository)
        Dim oDT As DataTable
        Try
            oDT = DLA2EAHelper.SQL2DataTable(String.Format("SELECT t_objectproperties.notes as config, t_objectproperties.property as name FROM t_object, t_objectproperties WHERE t_object.object_type = 'Artifact'  AND t_object.name = 'IDEAConfig' AND t_object.[version] >= '{0}' AND t_object.object_id = t_objectproperties.object_id", My.Application.Info.Version.ToString().Substring(0, 3)), Repository)
            If oDT.Rows.Count = 0 Then
            ...
            Else
                ...
                If Not CheckVersion() Then
                    MsgBox("You are using an older version please upgrade by installing a current version")
                End If
            End If
        Catch ex As Exception
            DLA2EAHelper.Error2Log(ex)
    ...
    Sub LoadDataSetXML(xmlstring As String, importstring As String, Tablename As String)
        Try
            ...
            End If
            If Tablename = "Settings" And Me.oIDEADefinitions.Tables(Tablename).Rows.Count <= 1 Then
                System.IO.Directory.CreateDirectory("c:\idea")
                Me.AddInitialSetting("PDFPackageTemplate", "Diagram Report", "HTML")
                Me.AddInitialSetting("PDFDiagramTemplate", "Diagram Report", "HTML")
                Me.AddInitialSetting("PDFElementTemplate", "Diagram Report", "HTML")
                ...
                Me.AddInitialSetting("DeduplicateConnectorsDiagram", "Select orig.name, orig.stereotype, orig.author, orig.version, dupl.name as duplicate_name, t_package.name as package_name, dupl.author as duplicate_author, orig.object_id as origid, dupl.object_id as duplid From t_object orig, t_package, t_object dupl, t_diagramobjects
...
order by 1  ", "DEDUPL")
                ...
                Me.AddInitialSetting("DeduplicateElementsDiagram", "Select orig.name, orig.stereotype, orig.author, orig.version, dupl.name as duplicate_name, 
            t_package.name as package_name, dupl.author as duplicate_author, orig.object_id as origid, dupl.object_id as duplid 
            ...
            and orig.version = dupl.version 
            and orig.object_id = t_diagramobjects.object_id
            and t_diagramobjects.diagram_id IN(#diagram_id#) order by 1 ", "DEDUPL")



                Me.AddInitialSetting("WasteBinPackage_id", "-999", "WASTEBIN")
                Me.AddInitialSetting("IDEAVersion", My.Application.Info.Version.ToString().Substring(0, 3), "VERSION")
                Me.AddInitialSetting("TaggedValuePrefix", "", "CONFIG")
            End If
            If Tablename = "SQL-Statement" And Me.oIDEADefinitions.Tables(Tablename).Rows.Count <= 1 Then
                'DMF raamwerk
                ...
                Me.AddInitialSQL("HousekeepingPDM", "Housekeeping",
                            ...
                            union 
                            select distinct dbo.nsda_gettaggedvalue(child_object.object_id, 'Owner') 
                            , child_object.object_name as [childname]
                            , dbo.nsda_gettaggedvalue(parent_object.object_id, 'Owner') 
                            , parent_object.object_name as [parentname]
